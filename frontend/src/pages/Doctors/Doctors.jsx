...
const Doctors = () => {
  ...
  //With this debounced useEffect, 
  // the API only fires after 700ms of no typing, making it more efficient.
  useEffect(() => {
        ...
        <div className="container">
          ...
          {!loading && !error && (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-5 ">
              {doctors.map((doctor) => (
                doctor.role ==="doctor" && <DoctorCard key={doctor._id} doctor={doctor} />
              ))}
            </div>
          )}
        </div>
